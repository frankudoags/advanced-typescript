export type CapitalizeKeys<T> = {
    [K in keyof T as Capitalize<string & K>]: T[K]
}

export type DeepReadOnly<T> = {
    readonly [K in keyof T]: T[K] extends object ? DeepReadOnly<T[K]> : T[K];
}

export type API = {
    getUser(): string
    config: {
        reload(): boolean
    }
}

export type Promisify<T> = {
    [K in keyof T]: T[K] extends (...args: any[]) => infer R
    ? (...args: any[]) => Promise<R>
    : T[K] extends object
    ? Promisify<T[K]>
    : T[K]
}

export type Prettify<T> = {
    [K in keyof T]: T[K];
} & {};

export type DeepPrettify<T> = T extends object
    ? T extends Function
    ? T
    : Prettify<{ [K in keyof T]: DeepPrettify<T[K]> }>
    : T;

export type PromiseAPI = Promisify<API>;


export type Flatten<T, P extends string = ''> = {
    [K in keyof T]: T[K] extends object
    ? Flatten<T[K], `${P}${K & string}.`>
    : { [Key in `${P}${K & string}`]: T[K] }
}[keyof T]

export type Merge<T> = {
    [K in keyof T]: T[K]
}

export type Input = {
    name: string
    user: {
        profile: {
            age: number
        }
    },

    brother: {
        age: number
    }
}

export type Output = DeepPrettify<Merge<Flatten<Input>>>;


export type PartialDeep<T> = {
    [K in keyof T]?: T[K] extends object
    ? PartialDeep<T[K]>
    : T[K];
}

export type Test = {
    name: number;
    company: {
        name: number;
        address: string;
    }
}
export type PartialTest = Partial<Test>;
export type PartialDeepTest = DeepPrettify<PartialDeep<Test>>;